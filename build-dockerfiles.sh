#!/bin/bash

set -o errexit

##############################################################
#                      Parse Arguments                       #
##############################################################

function usage {
cat << EOF
usage: $0 options

This script build Kong in different distributions

OPTIONS:
 -h      Show this message
 -k      Kong version to build
 -p      Platforms to target
 -t      Execute tests
 -r      Kong Fork Repo to pull from
EOF
}

ARG_PLATFORMS=
KONG_VERSION=
KONG_REPO=Mashape
TEST=false
while getopts “hk:r:p:t” OPTION
do
  case $OPTION in
    h)
      usage
      exit 1
      ;;
    k)
      KONG_VERSION=$OPTARG
      ;;
    p)
      ARG_PLATFORMS=$OPTARG
      ;;
    t)
      TEST=true
      ;;
    r)
      KONG_REPO=$OPTARG
      ;;
    ?)
      usage
      exit
      ;;
  esac
done

if [[ -z $ARG_PLATFORMS ]]; then
  usage
  exit 1
fi

# Check system
if ! [ "$(uname)" = "Darwin" ]; then
  echo "Run this script from OS X"
  exit 1
fi

##############################################################
#                      Check Arguments                       #
##############################################################

supported_platforms=( centos:5 centos:6 centos:7 debian:6 debian:7 debian:8 ubuntu:12.04.5 ubuntu:14.04.2 ubuntu:15.04 osx )
platforms_to_build=( )

for var in "$ARG_PLATFORMS"
do
  if [[ "all" == "$var" ]]; then
    platforms_to_build=( "${supported_platforms[@]}" )
  elif ! [[ " ${supported_platforms[*]} " == *" $var "* ]]; then
    echo "[ERROR] \"$var\" not supported. Supported platforms are: "$( IFS=$'\n'; echo "${supported_platforms[*]}" )
    echo "You can optionally specify \"all\" to build all the supported platforms"
    exit 1
  else
    platforms_to_build+=($var)
  fi
done

if [ ${#platforms_to_build[@]} -eq 0 ]; then
  echo "Please specify an argument!"
  exit 1
fi

echo "Building Kong $KONG_REPO/$KONG_VERSION: "$( IFS=$'\n'; echo "${platforms_to_build[*]}" )

##############################################################
#                        Start Build                         #
##############################################################

# Preparing environment
DIR=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )
echo "Current directory is: "$DIR
if [ "$DIR" == "/" ]; then
  DIR=""
fi


# Start build
for i in "${platforms_to_build[@]}"
do
  Dockerfilename="Dockerfile-$i"
  echo "Building for $i"
  sed -e "1s|.*|FROM $i|" Dockerfile
  sed -e "2s|.*|ENV KONG_VERSION $KONG_VERSION|" Dockerfile
  sed -e "3s|.*|ENV KONG_REPO $KONG_REPO|" Dockerfile
  echo "Building $Dockerfilename"
  if [ $? -ne 0 ]; then
    exit 1
  fi
done
